...
const BiometricManagement = ({ auth, devices = [] }) => {
    ...
    // Submit fetch logs with optional date range
    const submitFetchLogs = (e) => {
        ...
        const toastId = toast.loading(`Connecting to ${fetchLogsDevice.name} (${fetchLogsDevice.ip_address})...`, {
            autoClose: false
        });
        
        const requestPayload = {
            device_id: fetchLogsDevice.id,
            // Only include start and end dates if they are set
        ...
        .then(data => {
            if (data.success) {
                ...
                // Create a user-friendly message about the date range
                ...
                toast.update(toastId, {
                    render: `Successfully fetched logs${dateRangeMessage}: ${savedCount} new records saved`,
                    type: 'success',
                    isLoading: false,
                    autoClose: 5000
                });
